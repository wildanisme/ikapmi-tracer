    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white pt-4 dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div class="max-w-full overflow-x-auto">
            <table class="w-full">
                    ...
                    <tr>
                        <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">
                            <div class="flex items-center gap-3">
                                ...
                                >
                                    <svg :class="selectAll ? 'block' : 'hidden'" width="14" height="14" viewBox="0 0 14 14" fill="none">
                ...
                <tbody>
                    <tr v-for="row in tableRowData" :key="row.id" class="border-b border-gray-100 dark:border-white/[0.05]">
                        <td class="px-4 sm:px-6 py-3.5">
                            <div class="flex items-center gap-3">
                                ...
                                >
                                    <svg :class="selectedRows.includes(row.id) ? 'block' : 'hidden'" width="14" height="14" viewBox="0 0 14 14" fill="none">
                                        <path d="M11.6668 3.5L5.25016 9.91667L2.3335 7" stroke="white" stroke-width="1.94437" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </div>
                        ...
                        <td class="px-4 sm:px-6 py-3.5">
                            <div class="flex items-center gap-3">
                                <div class="flex items-center justify-center w-10 h-10 rounded-full font-medium text-sm" :class="[row.avatarBg, row.avatarColor]">
                        ...
                        <td class="px-4 sm:px-6 py-3.5">
                            <span class="text-theme-xs inline-block rounded-full px-2 py-0.5 font-medium" :class="getStatusClass(row.status)">
                                {{ row.status }}
                            </span>
                        </td>
                        <td class="px-4 sm:px-6 py-3.5">
                            <button type="button" @click="deleteRow(row.id)">
                                <svg class="text-gray-700 cursor-pointer size-5 hover:text-red-500 dark:text-gray-400 dark:hover:text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const handleSelectAll = () => {
    ...
    selectedRows.value = selectAll.value ? tableRowData.value.map((row) => row.id) : [];
};
...
const handleRowSelect = (id) => {
    ...
    selectAll.value = selectedRows.value.length === tableRowData.value.length;
};
...
const deleteRow = (id) => {
    if (confirm('Are you sure you want to delete this order?')) {
        tableRowData.value = tableRowData.value.filter((row) => row.id !== id);
        selectedRows.value = selectedRows.value.filter((rowId) => rowId !== id);
        selectAll.value = selectedRows.value.length === tableRowData.value.length;
    }
};
